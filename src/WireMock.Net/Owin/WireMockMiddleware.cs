...
internal class WireMockMiddleware(
    Next _, // Needed for being an ASP.NET Core's middleware
    ...
    IGuidUtils guidUtils)
{
    ...
    async Task InvokeInternalAsync(IContext ctx) {
        var request = await requestMapper.MapAsync(ctx.Request, options);
        ...
        try{
            foreach (var mapping in options.Mappings.Values.Where(m => m.Scenario is not null)){
                Debug.Assert(mapping.Scenario is not null);
                // Set scenario start
