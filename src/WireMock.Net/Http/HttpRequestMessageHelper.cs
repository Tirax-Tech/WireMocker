...
{
    static readonly IDictionary<string, bool> ContentLengthHeaderAllowed = new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase)
    {
    ...
    internal static HttpRequestMessage Create(IRequestMessage requestMessage, string url)
    {
        ...
        if (requestMessage.Headers != null && requestMessage.Headers.TryGetValue(HttpKnownHeaderNames.ContentType, out var messageHeader))
        {
            var value = messageHeader.FirstOrDefault();
            MediaTypeHeaderValue.TryParse(value, out contentType);
        }

        var bodyData = requestMessage.BodyData;
        httpRequestMessage.Content = bodyData?.BodyType switch
        {
